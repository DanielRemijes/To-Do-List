...
const initApp=()=>{
    ...
    clearItems.addEventListener("click", (event)=>{
        ...
        if (list.length) {
            const confirmed = confirm("Are you sure want to clear the entire list?");
            if (confirmed) {
                toDoList.clearList();
...
const loadListObject = ()=>{
    ...
    const parchedList = JSON.parse(storedList);
    parchedList.forEach(itemObj => {
...
const buildListItem = (item) => {
    ...
    check.id = item.getId();
    check.tabIndex =0;
    addClickListenerTouchbox(check);
...
const addClickListenerTouchbox = (checkbox) => {
